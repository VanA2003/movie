...
export default function MoviesRow(props) { 
    const { movies, title, isNextflix, idSection} = props
    const sliderRef = useRef()
    const movieRef = useRef() 
    const [dragDown, setDragDown] = useState(0)
    const [dragMove, setDragMovie] = useState(0)
  ...
  return (
    <MoviesRowContainer draggable='false' id={idSection}>
        ...
        <div className={`btnleft ${isNextflix && 'isNextflix'}`} onClick={handleScrollLeft}>
            <BsChevronLeft/>
        </div>
        <div className={`btnright ${isNextflix && 'isNextflix'}`} onClick={handleScrollRight}>
            <BsChevronRight/>
        </div>
    </MoviesRowContainer>
  )
